...
export function CategoryPage({ category, infantTab }: { category: GalleryCategory; infantTab?: InfantTab }) {
  ...
  return <div className={`category-page category-theme-${Math.max(categoryIndex, 0) % 6}`}><header className="category-header"><Logo /><Link href="/" className="back-link"><ArrowLeft size={15} /> voltar ao acervo</Link></header><main><section className="category-hero" style={{ backgroundImage: `linear-gradient(90deg, rgba(244,240,232,.98) 0%, rgba(244,240,232,.76) 45%, rgba(244,240,232,.16) 100%), url(${bannerTexture})` }}><div className="category-hero-copy"><p className="eyebrow">{infantTab ? "Infantil" : `Categoria ${String(categoryIndex + 1).padStart(2, "0")}`} <span className="hairline inline" /></p><h1>{pageTitle}</h1><p>{infantTab ? categoryIntros[infantSource[infantTab]] : categoryIntros[category.slug] ?? "Um pouco do que já fiz."}</p></div><span className="category-count">{photos.length} registros<br />{String(Math.max(categoryIndex + 1, 1)).padStart(2, "0")} / {String(galleryCategories.length).padStart(2, "0")}</span></section>{infantTab && <div className="infant-tabs" role="tablist" aria-label="Categorias infantis"><Link href="/infantil?aba=meninas" className={infantTab === "meninas" ? "active" : ""} role="tab" aria-selected={infantTab === "meninas"}>Meninas</Link><Link href="/infantil?aba=meninos" className={infantTab === "meninos" ? "active" : ""} role="tab" aria-selected={infantTab === "meninos"}>Meninos</Link><Link href="/infantil?aba=temas-diversos" className={infantTab === "temas-diversos" ? "active" : ""} role="tab" aria-selected={infantTab === "temas-diversos"}>Temas Diversos</Link></div>}<section className="category-gallery"><div className="gallery-heading"><div><p className="eyebrow">Trabalhos recentes <span className="hairline inline" /></p><h2>{infantTab ? `Infantil · ${infantTab === "temas-diversos" ? "Temas Diversos" : infantTab[0].toUpperCase() + infantTab.slice(1)}` : "Um pouco do que já fiz"}</h2></div><p>Detalhes que gosto de lembrar. <em>— {pageTitle}</em></p></div><div className="masonry-grid">{photos.map((photo, index) => <GalleryPhoto key={`${selectedCategory.slug}-${index}-${photo.src}`} {...photo} index={index} label={pageTitle} onOpen={() => setLightboxIndex(index)} />)}</div></section><nav className="category-pagination" aria-label="Navegação entre categorias"><Link href={`/${previous.slug}`}><ArrowLeft size={15} /><span>anterior<small>{previous.title}</small></span></Link><Link href="/" className="pagination-center">índice</Link><Link href={`/${next.slug}`}><span>próxima<small>{next.title}</small></span><ArrowRight size={15} /></Link></nav></main><footer className="category-footer"><Logo compact /><span>Dalva Bolos · acervo autoral</span><Link href="/">voltar ao acervo ↑</Link></footer>{lightboxIndex !== null && <div className="lightbox" role="dialog" aria-modal="true" aria-label="Imagem ampliada" onClick={() => setLightboxIndex(null)}><button className="lightbox-close" onClick={() => setLightboxIndex(null)} aria-label="Fechar ampliação"><X size={22} /></button><button className="lightbox-arrow left" onClick={(event) => { event.stopPropagation(); setLightboxIndex((lightboxIndex - 1 + photos.length) % photos.length); }} aria-label="Imagem anterior"><ArrowLeft size={22} /></button><div className="lightbox-inner" onClick={(event) => event.stopPropagation()}><SmartImage src={photos[lightboxIndex].src} fallback={fallbackDetail} alt={`${pageTitle} — imagem ampliada`} /><div className="lightbox-meta"><span>{String(lightboxIndex + 1).padStart(2, "0")} / {String(photos.length).padStart(2, "0")}</span><span>{pageTitle}</span></div></div><button className="lightbox-arrow right" onClick={(event) => { event.stopPropagation(); setLightboxIndex((lightboxIndex + 1) % photos.length); }} aria-label="Próxima imagem"><ArrowRight size={22} /></button></div>}</div>;
}

export function InfantilPage() {
  const [location] = useLocation();
  const value = new URLSearchParams(window.location.search).get("aba") as InfantTab | null;
  void location;
  const tab: InfantTab = value && ["meninas", "meninos", "temas-diversos"].includes(value) ? value : "meninas";
  const base = galleryCategories.find((item) => item.slug === "infantil-feminino")!;
  return <CategoryPage category={base} infantTab={tab} />;
}
